...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['dentist', 'dental-assistant', 'office-manager', 'receptionist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PracticeInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/practices` : null),
    () => getPractices({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Practices',
      path: isTenantUser ? `/practices/view/${data?.[0]?.id}` : '/practices',
      entity: 'practice',
