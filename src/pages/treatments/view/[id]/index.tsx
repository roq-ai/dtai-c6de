...
import { getTreatmentById } from 'apiSdk/treatments';
import { TreatmentInterface } from 'interfaces/treatment';
import { DiagnosisListPage } from 'pages/diagnoses';

function TreatmentViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TreatmentInterface>(
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('treatment', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/treatments/edit/${id}`} passHref legacyBehavior>
                    <Button
