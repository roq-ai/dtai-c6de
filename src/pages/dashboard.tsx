...
import { TreatmentListPage as ShowCaseList } from 'pages/treatments';

function DashboardPage() {
  const { session } = useSession();

  const treatmentStat = useSWR<PaginatedInterface<TreatmentInterface>>(
    () => `/treatments`,
    () => getTreatments({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Treatments',
      result: treatmentStat?.data?.totalCount,
      isLoading: treatmentStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`treatments`} passHref legacyBehavior>
                  <Button
