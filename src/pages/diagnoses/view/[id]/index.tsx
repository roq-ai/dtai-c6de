...
import { getDiagnosisById } from 'apiSdk/diagnoses';
import { DiagnosisInterface } from 'interfaces/diagnosis';

function DiagnosisViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<DiagnosisInterface>(
    () => (id ? `/diagnoses/${id}` : null),
    () =>
      getDiagnosisById(id, {
        relations: ['treatment'],
      }),
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('diagnosis', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/diagnoses/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('treatment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Treatment"
                    text={
                      <Link as={NextLink} href={`/treatments/view/${data?.treatment?.id}`}>
                        {data?.treatment?.name}
                      </Link>
                    }
                  />
