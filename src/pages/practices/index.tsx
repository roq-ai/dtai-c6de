...
type ColumnType = ColumnDef<PracticeInterface, unknown>;
...
export function PracticeListPage(props: PracticeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PracticeInterface>>(
    () => `/practices?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PracticeInterface) => {
    if (hasAccess('practice', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/practices/view/${row.id}`);
    }
  };
